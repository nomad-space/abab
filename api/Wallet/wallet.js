...
const error = require('../../app/modules/error/api');
...
module.exports = (API, redis) => {
    ...
    API.register('wallet', (user, param) => {
        return new Promise((resolve, reject) => {

            param.type = 'ETH';
            if (!web3.isConnected()) {
                return reject(API.error.create('Web3 is not connected to geth server. Please try latter you request', 'web3', {
    ...
    API.register('requestFunctionContract', (user, param) => {
        return new Promise((resolve, reject) => {


            if (!param.from || typeof param.from !== 'string')
                return reject(API.error.create('Request param "from" incorrect', 'param', {
                    pos: 'api/wallet.js(requestFunctionContract):#1',
                    param: param
                }, 0));
            if (!param.function || typeof param.function !== 'string')
                return reject(API.error.create('Request param "function" incorrect', 'param', {
                    pos: 'api/wallet.js(requestFunctionContract):#2',
                    param: param
                }, 0));
            if (!param.param || typeof param.param !== 'string')
                return reject(API.error.create('Request param "param" incorrect', 'param', {
                    pos: 'api/wallet.js(requestFunctionContract):#3',
                    param: param
                }, 0));

            if (!web3.isAddress(param.from)) return reject(API.error.create('Request param "from" is not valid Address', 'param', {
            ...
            if (!_.find(sol_config._abi, {name: param.function}) || _.find(sol_config._abi, {name: param.function}).type !== 'function')
                return reject(API.error.create('Request param "function" is not found function contract', 'param', {
                    pos: 'api/wallet.js(requestFunctionContract):#5',
                    param: param
                }, 0));
            ...
            db.wallets.findOne({
                ...
                db.tx.find({
                    ...
                    if (solidityFunction._inputTypes.length !== param.param.length) {
                        return reject(API.error.create('invalid req param solidityFunction param: ' + JSON.stringify(solidityFunction._inputTypes), 'param', {
                    ...
                    web3.eth.sendRawTransaction('0x' + serializedTx.toString('hex'), function (err, hash) {
                        if (err) {
                            console.error('Error sendRawTransaction:', err);
                            return reject(API.error.create('Error send TX', 'web3', err, 0));
                        }
                        else {
                            ...
                            new db.tx({
                                ...
                                return reject(API.error.create(err.message, 'db', err, 3));
                ...
                return reject(API.error.create(err.message, 'db', err, 6));
    ...
    API.register('transfer_coin', (user, param) => {
        // transfer abab_coin
        return new Promise((resolve, reject) => {

            return reject(API.error.create('This method in development', 'server', {
    ...
    API.register('callFunctionContract', true, (user, param) => {
        return new Promise((resolve, reject) => {

            let contract = web3.eth.contract(sol_config._abi).at(sol_config._address);
            if (!_.find(sol_config._abi, {name: param.function}) || !contract[param.function]) {
                return reject(API.error.create('Call function error function not found', 'param', {
                    param: param,
                    find_fn: _.find(sol_config._abi, {name: param.function})
                }, 0));
            }
            if (!param.param || typeof param.param !== 'object') {
                return reject(API.error.create('Param type is not array', 'param', {
                    param: param,
                    find_fn: _.find(sol_config._abi, {name: param.function})
                }, 0));
            }
            ...
            paramContract.push(function (err, result) {
                if (err)
                    return reject(API.error.create('Param type is not array', 'contract', {
                        param: param,
                        find_fn: _.find(sol_config._abi, {name: param.function})
                    }, 2));
    ...
    if (web3.isConnected()) {
        ...
        let events = contract.allEvents({fromBlock: config.get('geth:lastBlockNumber'), toBlock: 'latest'});
